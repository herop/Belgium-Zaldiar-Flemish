...
iScroll.prototype = {
	...
	toReset: function (resetX, resetY) {
		var that = this;
	...
	touchMove: function(e) {
		...
		var that = this,
			...
			newCoord = function (a, delta, newP, max) {
				return that.options.bounce ? Math.round(a + delta / 3) : newP >= 0 ? 0 : max;
			};
		...
		if (newX >= 0 || newX < that.maxScrollX) {
			newX = newCoord(that.x, leftDelta, newX, that.maxScrollX);//that.options.bounce ? Math.round(that.x + leftDelta / 3) : newX >= 0 ? 0 : that.maxScrollX;
		}
		if (newY >= 0 || newY < that.maxScrollY) { 
			newY = newCoord(that.y, topDelta, newY, that.maxScrollY);//that.options.bounce ? Math.round(that.y + topDelta / 3) : newY >= 0 ? 0 : that.maxScrollY;
		}
	...
	touchEnd: function(e) {
		...
		if ((!that.options.snap && time > 250) || !that.moved) {
			that.resetPosition();
			return;
		}
		...
		if (that.options.momentum) {
			...
			if (that.scrollX) {
				momentumX = that.createMomentum(time, 'x');
			} else {
				momentumX = { dist: 0, time: 0 };
			}
			if (that.scrollY) {
				momentumY = that.createMomentum(time, 'y');
			} else {
	...
	createMomentum: function (time, axis) {
		var that = this;
		if (axis === 'x'){
			return that.momentum(that.x - that.scrollStartX,
								time,
			...
			return that.momentum(that.y - that.scrollStartY,
								time,
	...
	scrollTo: function (destX, destY, runtime) {
		...
		this.moved = true;
		this.setTransitionTime(runtime || '350ms');
